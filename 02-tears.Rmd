...
combining training and testing data back together
```{r}
...
rejects= rejected %>% 
  ...
  mutate(PERS_H= ifelse(PERS_H>=7, '7+', PERS_H),
         ...
         CARDS= ifelse(CARDS=='VISA Others', 'Other credit car', CARDS),
         CARDS= ifelse(CARDS=='American Express', 'Other credit car', CARDS),
         CARDS= ifelse(CARDS=='VISA Citibank', 'Other credit car', CARDS)) #citibank in rejects and not accepts and need to get it to match
...
#getting the scores for rejects data from our initial model we made
```{r}
pdo <- 50
score <- 500
odds <- 20
...
# Add Scores to Train Comb from Final Model #
